...
def train(path=False):
    ...
    employee = os.path.basename(path)
    preprocess(path, height, width, data, labels, face_cascade, employee)
     
    # Load existing labels from labels.json
    with open("labels.json", "r") as file:
        old_labels = json.load(file)
            
    old_labels.append(employee)
        
    # Saving labels
    with open("labels.json", "w") as file:
        json.dump(sorted(set(old_labels)), file)
            
    num_classes = len(set(old_labels))

    # Split Data
    X_train, X_test, y_train_names, y_test_names = train_test_split(data, labels, test_size=0.2, random_state=42)
    X_train, X_val, y_train_names, y_val_names = train_test_split(X_train, y_train_names, test_size=0.2, random_state=42)
    ...
    # Mapp employee names to numerical labels
    ...
    # Freeze the first three pairs of convolutional and max-pooling layers
    ...
    # Remove the last layer and add a new Dense layer with updated number of classes
    model.pop()
    new_layer_name = f"new_output_layer_{datetime.now().strftime('%Y%m%d%H%M%S')}"
    model.add(Dense(num_classes, activation='softmax', name=new_layer_name))
    ...
    # Save the model
    model.save("models/TLModel.keras")
